...
import { STOCK_PRICE_DIR, KLINE_DIR, PROJECT_ROOT, bjDateString } from './shared';
...
const SA_DATA_DIR = path.join(PROJECT_ROOT, 'data', 'sa');
const SA_SENTIMENT_DIR = path.join(SA_DATA_DIR, 'sentiment');


const LB_SCRIPT_DIR = STOCK_PRICE_DIR;

// Helper: run a Longbridge Python script and return parsed JSON
async function runLongbridgeScript(pythonScript: string): Promise<any> {
...
LBEOF`;
  try {
    ...
    if (stderr) logError('Python stderr', { stderr: stderr.slice(0, 500) });
    const jsonStartBrace = stdout.indexOf('{');
    const jsonStartBracket = stdout.indexOf('[');
    let jsonStart: number;
    if (jsonStartBrace === -1) jsonStart = jsonStartBracket;
    else if (jsonStartBracket === -1) jsonStart = jsonStartBrace;
    else jsonStart = Math.min(jsonStartBrace, jsonStartBracket);
...
function toLBSymbol(code: string): string {
  if (code.includes('.')) return code;
  return `${code}.US`;
}

// GET /api/sa/stock/:symbol - Fetch stock profile + quote + 12M candle via Longbridge
router.get('/sa/stock/:symbol', async (req: Request, res: Response) => {
...
load_dotenv('${STOCK_PRICE_DIR}/.env')
...
router.get('/sa/candles/:symbol', async (req: Request, res: Response) => {
  const symbol = String(req.params.symbol).trim();
...
router.get('/sa/indicators/:symbol', async (req: Request, res: Response) => {
  const symbol = String(req.params.symbol).trim();
  const period = String(req.query.period || 'day').trim().toLowerCase();
...
async function searchNewsZhipu(query: string): Promise<{ title: string; url: string; snippet: string }[]> {
  const apiKey = process.env.ZHIPU_API_KEY;
  if (!apiKey) throw new Error('Missing ZHIPU_API_KEY');
...
async function analyzeWithDeepSeek(
  ...
  const priceSummary = recent.map(c =>
    `${c.date}: Open=${c.open}, High=${c.high}, Low=${c.low}, Close=${c.close}, Vol=${c.volume}`
  ).join('\n');
...
router.get('/sa/sentiments', (_req: Request, res: Response) => {
  try {
    if (!fs.existsSync(SA_SENTIMENT_DIR)) {
      res.json([]);
      return;
    }
    const files = fs.readdirSync(SA_SENTIMENT_DIR).filter(f => f.endsWith('.json'));
    const result: any[] = [];
    for (const f of files) {
      try {
        const raw = JSON.parse(fs.readFileSync(path.join(SA_SENTIMENT_DIR, f), 'utf8'));
        result.push({
          symbol: raw.symbol,
          date: raw.date,
...
router.get('/sa/sentiment/:symbol', (req: Request, res: Response) => {
  ...
  try {
    ...
    const files = fs.readdirSync(SA_SENTIMENT_DIR)
    ...
    const raw = JSON.parse(fs.readFileSync(path.join(SA_SENTIMENT_DIR, files[0]), 'utf8'));
    res.json(raw);
  } catch (err: any) {
...
router.post('/sa/sentiment/:symbol', async (req: Request, res: Response) => {
  const symbol = String(req.params.symbol).trim();
  if (!symbol) {
    res.status(400).json({ error: 'Symbol is required' });
    return;
  ...
  const cacheFile = path.join(SA_SENTIMENT_DIR, `${symbol.toUpperCase()}-${today}.json`);
  ...
  try {
    ...
    let news: { title: string; url: string; snippet: string }[] = [];
    try {
    ...
    // Save to cache.
    try {
      fs.mkdirSync(SA_SENTIMENT_DIR, { recursive: true });
      fs.writeFileSync(cacheFile, JSON.stringify(result, null, 2));
      logInfo('S-A sentiment cached', { symbol: symbol.toUpperCase(), date: today });
    } catch (err) {
